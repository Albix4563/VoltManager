...
const catalogPath = new URL('../src/VoltManager/wwwroot/js/i18n.catalogs.js', import.meta.url);
...
test('update suspension uses the host snooze deadline and can resume immediately', () => {
    ...
    assert.match(catalogSource, /Manual update checks remain available/);
});

test('settings copy reports the fifteen-minute automatic cadence', () => {
    assert.match(catalogSource, /every 15 minutes/);
    assert.match(catalogSource, /ogni 15 minuti/);
});
