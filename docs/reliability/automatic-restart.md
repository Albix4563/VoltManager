...
.\build.ps1 -SkipTests -SkipInstaller
```
...
- tests simulate child-process lifetime and time; they do not intentionally crash the GitHub Actions runner desktop session;
- a manual Windows smoke test is still required to verify UAC inheritance, tray shutdown, installer payload and Task Scheduler behavior on a real interactive machine;
